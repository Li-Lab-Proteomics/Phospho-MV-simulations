...
mod_listPcaMethods <- function(which=c("all", "linear", "nonlinear")) {
  switch(match.arg(which),
         all={
           return(c("svd", "nipals", "rnipals", "mod_bpca", "ppca",
...
mod_pca <- function(object, method, nPcs=2, 
  ...
  if (!mod_checkData(Matrix, verbose=interactive()))
  ...
  method <- match.arg(method, choices=mod_listPcaMethods())
  
  prepres <- mod_prep(Matrix, scale=scale, center=center, simple=FALSE, ...)
  ...
  switch(method,
         ...
         mod_bpca={
           res <- mod_bpca(prepres$data, nPcs=nPcs, ...) 
         },
         ppca={
  ...
  if (completeObs) {
    cObs <- Matrix
    if(method %in% mod_listPcaMethods("nonlinear"))
...
# nni: only this 1 place
mod_nni <- function(object, method=c("llsImpute"), subset=numeric(), ...) {
  isExprSet <- FALSE
  ...
  if ( !mod_checkData(as.matrix(object), verbose=interactive()) )
...
mod_plotPcs <- function(object,
  ...
  panel <- function(x,y, ...) {
    ...
    if(!is.null(hotelling)) {
      A <- length(pcs)
      el <- mod_simpleEllipse(x, y, alfa=hotelling)
      lines(el)
    }
    if(is.null(sl))
      points(x, y, ...)
...
##' @keywords multivariate
##' @author Henning Redestig
# svdPca: only this 1 place
mod_svdPca <- function(Matrix, nPcs=2, 
...
mod_simpleEllipse <- function(x, y, alfa=0.95, len=200) {
  N <- length(x)
  A <- 2
