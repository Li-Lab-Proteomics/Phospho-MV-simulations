...
# scores.pcaRes: only this 1 place --- only 2 in this .R file
mod_scores.pcaRes <- function(object, ...) object@scores
##' Get scores from a pcaRes object
##' @param object a pcaRes object
##' @param ... not used
##' @return The scores as a matrix
...
##' @author Henning Redestig
setMethod("scores", "pcaRes", mod_scores.pcaRes)
...
mod_loadings.pcaRes <- function(object, ...) object@loadings
##' Get loadings from a pcaRes object
##' @param object a pcaRes object
##' @param ...  not used
##' @return The loadings as a matrix
...
setMethod("loadings", "pcaRes", mod_loadings.pcaRes)
...
##' @export 
##' @author Henning Redestig
# dim.pcaRes: only this 1 place
mod_dim.pcaRes <- function(x)  {
...
##' @keywords multivariate
##' @name biplot-methods
mod_biplot.pcaRes <- function(x, choices=1:2, scale=1, pc.biplot=FALSE, ...) {
  if(length(choices)!=2)
...
##' @name biplot-methods
setMethod("biplot", "pcaRes", mod_biplot.pcaRes)
...
setMethod('R2VX', 'pcaRes',
  ...
  dat <- mod_prep(data, scale=scl(object), center=center(object))
...
mod_summary.pcaRes <- function(object, ...){
...
setMethod("summary", "pcaRes", mod_summary.pcaRes)
...
##' @author Henning Redestig
mod_predict.pcaRes <- function(object, newdata, pcs=nP(object),
                           pre=TRUE, post=TRUE, ...) {
  if(!method(object) %in% mod_listPcaMethods("linear"))
    stop("predict method not implemented for that type of PCA")
  if(pre)
    newdata <- mod_prep(newdata, scl(object), center(object))
  ...
  if(post)
    xhat <- mod_prep(xhat, scl(object), center(object), reverse=TRUE)
  list(scores=tnew, x=xhat)
}
## @importFrom stats predict
##' @name predict-methods
##' @aliases predict,pcaRes-method
setMethod("predict", "pcaRes", mod_predict.pcaRes)
...
##' @author Henning Redestig
mod_residuals.pcaRes <- function(object, data=completeObs(object), ...) {
  if(is.null(data))
...
##' @name rediduals-methods
setMethod("residuals", "pcaRes", mod_residuals.pcaRes)
##' @name rediduals-methods
##' @aliases resid,pcaRes-method
setMethod("resid", "pcaRes", mod_residuals.pcaRes)
...
##' @name fitted-methods
##' @author Henning Redestig
mod_fitted.pcaRes <- function(object, data=NULL, nPcs=nP(object),
                          pre=TRUE, post=TRUE, ...) {
  if(method(object) %in% mod_listPcaMethods("nonlinear")) {
    ...
    if(pre)
      data <- mod_prep(data, scl(object), center(object))
    recData <-
      mod_errorHierarchic(object@network, t(scores(object)), t(data))$out[,,nPcs]
  ...
  if(post)
    recData <- mod_prep(recData, scl(object), center(object), reverse=TRUE)
  return(recData)
}
## @importFrom stats fitted
##' @name fitted-methods
##' @aliases fitted,pcaRes-method
setMethod("fitted", "pcaRes", mod_fitted.pcaRes)
...
##' @export
##' @author Henning Redestig
mod_plot.pcaRes <- function(x, y=NULL, main=deparse(substitute(object)),
                        col=gray(c(0.9, 0.5)), ...) {
  y <- NULL
...
setMethod("plot", "pcaRes", mod_plot.pcaRes)
...
function(object, pcs=c(1,2), scoresLoadings=c(TRUE, TRUE),
  ...
  if(method(object) %in% mod_listPcaMethods("nonlinear") && scoresLoadings[2])
    scoresLoadings[2] <- FALSE
  
  if(length(pcs) > 2)
    mod_plotPcs(object, pcs, scoresLoadings=scoresLoadings,...)
  else {
    if(is.null(sub))
    ...
    if(scoresLoadings[1]) {
      ...
      if(!is.null(hotelling)) {
        A <- length(pcs)
        el <- mod_simpleEllipse(scores(object)[,pcs[1]],
